// backup-dispatcher.deployed.js
//
// SNAPSHOT OF DEPLOYED CODE — not build source, and not imported by anything here.
//
// This is the esbuild bundle actually running as the `job-scout-backup-dispatcher`
// Cloudflare Worker: the thing that composes and sends every match email. It is
// deployed from the Cloudflare dashboard and has no source repository, which is
// precisely how it drifted out of sync with email-template.html on 2026-08-10 and
// dropped every match email to plain text. It is checked in here so that drift is
// visible in review instead of invisible until an inbox shows it.
//
...
//
// Fetched and redeployed 2026-08-11 with two fixes:
...
//     -F 'worker.js=@file.js;filename=worker.js;type=application/javascript+module'
//
// Known gap: no apply-link resolution. Sent email still links job.url, so PR #18
// (real apply links) benefits the dashboard only. See JOB_SOURCING_MATCHING_HANDOFF.md.
...
async function handleSendEmail(request, env) {
  ...
  let emailPayload;
  try {
    emailPayload = emailTemplate ? buildEmail({ template: emailTemplate, candidate, jobs, now }) : fallbackEmail({ candidate, jobs });
  } catch (error) {
    templateError = String(error.message || error).slice(0, 160);
    emailPayload = fallbackEmail({ candidate, jobs });
  }
  try {
...
async function dispatch(env, { dryRun = false, force = false } = {}) {
  ...
  for (const candidate of selected) {
    ...
    try {
      ...
      let emailPayload;
      try {
        emailPayload = emailTemplate ? buildEmail({ template: emailTemplate, candidate, jobs: unique, now }) : fallbackEmail({ candidate, jobs: unique });
      } catch (error) {
        console.error("Falling back to plain-text email:", String(error.message || error).slice(0, 160));
        emailPayload = fallbackEmail({ candidate, jobs: unique });
      }
      await sendEmail(env, {
        to: candidate.email,
