...
var cleanInputString = (value) => typeof value === "string" ? value.trim() : "";
var hasCompleteJobBrief = (job) => [job?.job_summary, job?.match_reason, job?.key_requirements].every(
...
async function handleSendEmail(request, env) {
  ...
  if (!jobs.length) {
    return json({
      ok: false,
      error: "no_valid_jobs: each job needs title, company, an http(s) url, and a complete job brief"
    }, 400);
  }
